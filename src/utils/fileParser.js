...
export const extractTextFromPDF = async (file) => {
  try {
    ...
    if (error.name === 'PasswordException') {
      ...
      throw new Error('No text could be extracted. The PDF might be scanned or image-based.');
    } else if (error.message.includes('worker failed') || error.message.includes('Worker') || error.message.includes('Failed to fetch')) {
      throw new Error('PDF processing failed. Please try converting to a plain text format or try another file.');
    }
...
export const extractTextFromDOCX = async (file) => {
  try {
    ...
    if (!result) {
      ...
      for (let i = 0; i < uint8Array.length; i++) {
        ...
        if (byte >= 32 && byte <= 126) {
          ...
          if (textBuffer.length > 5) { // Only keep text fragments of reasonable length
            result += textBuffer + ' ';
          }
          textBuffer = '';
          inText = false;
    ...
    throw new Error('Failed to extract text from DOCX file. Please try converting to PDF or TXT format.');
  }
};
