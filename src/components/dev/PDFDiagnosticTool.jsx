  ...
  return (
    <div className="p-4 border border-gray-300 rounded-lg bg-white shadow">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {Object.keys(results).map(methodName => (
          <div key={methodName} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
            ...
            <div className="space-y-2 text-sm">
              <p><span className="font-medium">Time:</span> {results[methodName].executionTimeMs}ms</p>
              <p><span className="font-medium">Characters:</span> {results[methodName].charCount}</p>
              <p><span className="font-medium">Words:</span> {results[methodName].wordCount}</p>
              <div>
        ...
        {/* Errors */}
        {Object.keys(errors).filter(k => errors[k]).map(methodName => (
          <div key={methodName} className="border border-red-200 rounded-lg p-4 bg-red-50">
            <h3 className="font-semibold text-lg mb-2 text-red-700 capitalize">{methodName} Error</h3>
            <div className="space-y-2 text-sm">
